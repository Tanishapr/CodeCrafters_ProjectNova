    ...
    if st.button("Ingest Data"):
        if username and password and db_name:
            ...
            try:
                ...
                st.subheader("Bias Analysis - Before Mitigation")
                st.dataframe(bias_before)
                st.bar_chart(bias_before)

                st.subheader("Bias Analysis - After Mitigation")
                st.dataframe(bias_after)
                st.bar_chart(bias_after)
...
# with tab2:
#     st.header("Predict Nova Credit Score")

#     # Collect user input
#     user_data = {
#         "earnings_weekly": st.number_input("Earnings Weekly", min_value=0.0),
#         "trip_count_monthly": st.number_input("Trip Count Monthly", min_value=0),
#         "avg_rating_last_90_days": st.number_input("Average Rating (last 90 days)", min_value=0.0, max_value=5.0, step=0.01),
#         "cancellation_rate": st.number_input("Cancellation Rate (%)", min_value=0.0, max_value=100.0, step=0.1),
#         "age": st.number_input("Age", min_value=18, max_value=100),
#         "rent_paid_last_12_months": st.number_input("Rent Paid Last 12 Months", min_value=0.0),
#         "utility_paid_last_12": st.number_input("Utility Paid Last 12 Months", min_value=0.0),
#         "app_logins_last_30_days": st.number_input("App Logins Last 30 Days", min_value=0),
#         "complaints_last_6_months": st.number_input("Complaints Last 6 Months", min_value=0),
#         "outstanding_loans": st.number_input("Outstanding Loans", min_value=0.0),
#     }

#     if st.button("Predict Credit Score"):
#         try:
#             prediction = predict_credit_score(user_data)
#             st.success(f"Predicted Nova Credit Score: {prediction:.2f}")
#         except Exception as e:
